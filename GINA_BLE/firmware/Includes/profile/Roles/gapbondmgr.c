...
static void gapBondMgrBondReq( uint16 connHandle, uint8 idx, uint8 stateFlags, uint8 role );
...
bStatus_t GAPBondMgr_LinkEst( uint8 addrType, uint8 *pDevAddr, uint16 connHandle, uint8 role )
{
  ...
  if ( idx < GAP_BONDINGS_MAX )
  {
    ...
    // If peripheral, load the key information for the bonding
    // If central and initiaiting security, load key to initiate encyption
    ...
    case GAP_SLAVE_REQUESTED_SECURITY_EVENT:
      {
        ...
        if (pLink != NULL && gapBond_PairingMode != GAPBOND_PAIRING_MODE_INITIATE)
        {
          ...
          if ( idx < GAP_BONDINGS_MAX )
          {
            gapBondMgrBondReq( connHandle, idx, gapBondMgrGetStateFlags( idx ), GAP_PROFILE_CENTRAL );
          }
...
static uint8 gapBondMgrFindReconnectAddr( uint8 *pReconnectAddr )
{
  ...
  for ( uint8 idx = 0; idx < GAP_BONDINGS_MAX; idx++ )
  {
    ...
    if ( osal_memcmp( bonds[idx].reconnectAddr, pReconnectAddr, KEYLEN ) )
    {
      return ( idx ); // Found it
    }
  }
