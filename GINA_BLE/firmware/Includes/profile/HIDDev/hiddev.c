...
// TRUE if device has bonded
static uint8 hidDevBonded = FALSE;
 ...
 * @return  bStatus_t
 */
bStatus_t HidDev_SetParameter( uint8 param, uint8 len, void *value )
{
 ...
 * @return  bStatus_t
 */
bStatus_t HidDev_GetParameter( uint8 param, void *value )
{
  bStatus_t ret = SUCCESS;
                         ...
                         uint8 *pValue, uint8 *pLen, uint16 offset, uint8 maxLen )
{
  ...
  else if ( uuid == HID_EXT_REPORT_REF_UUID )
  {
...
static void hidDevDisconnected( void )
{
  ...
  if ( hidDevBonded && (pHidDevCfg->hidFlags & HID_FLAGS_NORMALLY_CONNECTABLE) )
  {
  ...
  else if ( state == GAPBOND_PAIRING_STATE_COMPLETE )
  {
    ...
    if ( status == SUCCESS )
    {
      ...
      if ( (pItem = linkDB_Find( gapConnHandle )) != NULL )
      {
        ...
        hidDevBonded = ( (pItem->stateFlags & LINK_BOUND) == LINK_BOUND );
