/**************************************************************************************************
  Filename:       osal_snv.c
  Revised:        $Date: 2011-11-09 15:37:06 -0800 (Wed, 09 Nov 2011) $
  Revision:       $Revision: 28302 $
...
// TO reduce code size and for CC2533, actual ADC check can be removed.
#ifdef HAL_MCU_CC2530
# define  OSAL_NV_CHECK_BUS_VOLTAGE  (HalAdcCheckVdd( HAL_ADC_VDD_LIMIT_4 ))
#elif defined HAL_MCU_CC2533
// TODO: Add battery monitoring code when the chip is functional.
// For now, CC2533 does not support battery monitoring correctly.
...
void osal_snv_init( void )
{
  if (!initNV())
  {
    ...
    // TODO: return failure in case HAL ASSERT is not turned on?
  }
}
