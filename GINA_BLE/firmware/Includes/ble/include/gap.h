/**
  @headerfile:    gap.h
  $Date: 2012-03-21 15:37:48 -0700 (Wed, 21 Mar 2012) $
  $Revision: 29841 $
...
#define TGAP_LIM_ADV_TIMEOUT           1  //!< Maximum time to remain advertising, when in Limited Discoverable mode (mSec)
...
#define TGAP_FILTER_ADV_REPORTS       33  //!< Filter duplicate advertising reports. Default TRUE.
  ...
  #define TGAP_GAP_TESTCODE           34  //!< GAP TestCodes - puts GAP into a test mode
  #define TGAP_SM_TESTCODE            35  //!< SM TestCodes - puts SM into a test mode
  #define TGAP_AUTH_TASK_ID           36  //!< Task ID override for Task Authentication control (for stack internal use only)
  #define TGAP_PARAMID_MAX            37  //!< ID MAX-valid Parameter ID
#else
  #define TGAP_AUTH_TASK_ID           34  //!< Task ID override for Task Authentication control (for stack internal use only)
  #define TGAP_PARAMID_MAX            35  //!< ID MAX-valid Parameter ID
#endif
 ...
 */
   ...
   *              FAILURE: not workable.<BR>
   */
  extern bStatus_t GAP_Bond( uint16 connectionHandle, uint8 authenticated, smSecurityInfo_t *pParams );
